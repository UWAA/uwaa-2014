...
class HuskiesVoteEvents extends ThumbnailBrowser implements Thumbnail
{
    ...
    $args = array (
      ...
      'tax_query' => array(
         array(
          ...
          'field'    => 'slug',
          'terms'    => array( 'huskies-vote-event')
          ),
      ), //End tax query
      );
  ...
  public function extractPostInformation($query)
  {
      ...
      if ($this->currentPostID == get_the_ID() ) {
        ...
        $this->postImageThumbnailURL = $this->UI->returnPostFeaturedImageURL(get_post_thumbnail_id(get_the_ID()), 'postExcerptRowOfFour');
...
public function displayNothing() {
    echo '<div class="col-sm-12">No upcoming events.</div>';
}
