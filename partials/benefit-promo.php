...
if (!empty($promoText)) {
    ...
    if ($UWAA->Memberchecker->isLoggedIn == true && $UWAA->Memberchecker->hasActiveMembership == false) 
    {
     ...
     elseif ($UWAA->Memberchecker->isLoggedIn == true && $UWAA->Memberchecker->membershipType == "Life Member" && get_post_meta(get_the_ID(), 'mb_benefit_promotion_life', true) != '' ) {
        echo do_shortcode(wp_kses(get_post_meta(get_the_ID(), 'mb_benefit_promotion_life', true), array('a' => array(
            'href' => array(),
            'title' => array(),
            'class' => array()
        ), 'div' => array(
